...
- Support HTTP/HTTPS/SOCKS5 proxy for all upstream requests via standard `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` environment variables

### Bug Fixes

- Keep `/v1/models` compatible with Copilot's usage-based billing transition by using current model-access headers and retrying with the developer CLI catalog identity when the upstream returns the billing update notice
